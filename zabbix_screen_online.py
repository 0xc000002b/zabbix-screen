...
def getNames(host_list,url,auth,graphtype=0 ,dynamic=0):
    if (graphtype == 0):
       selecttype = ['graphid']
       select = 'selectGraphs'
    if (graphtype == 1):
       ...
       select = 'selectItems'
    values=({'jsonrpc' : '2.0',
             'method' : 'graph.get',                                                                                  'params' : {
                  'output' : ['graphid','name'],                                                                           select : [selecttype,'name'],
                  'hostids' : host_list,                                                                                   'sortfield' : 'name',
                  },
             'auth' : auth,
             'id' : 3
              })
    ...
    name_list = []
    for name in output:
        if 'Network' in  name['name']:
            name_list.append(name['name'])
    name2_list = list(set(name_list))
    return name2_list

#定义获取graphid的函数
def getGraphs(host_list,names_list, url, auth, columns, graphtype=0 ,dynamic=0):
    if (graphtype == 0):
       selecttype = ['graphid']
       select = 'selectGraphs'
    if (graphtype == 1):
    ...
    bb = sorted(output_list,key = lambda x:x['graphid'])
    graphs = []
    if (graphtype == 0):
        for i in bb:
            print i
...
def main():
    url = 'http://zbx.test.com/api_jsonrpc.php'
    username = 'test'
    password = '1122334455'
    auth = authenticate(url, username, password)
    host_list = getHosts(url,auth)
    names_list = getNames(host_list,url,auth)
    print names_list
#    graph_ids = getGraphs(host_list,graphname, url, auth, columns)
    graph_ids = getGraphs(host_list,names_list, url, auth, columns)
